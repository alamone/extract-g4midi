...
if __name__ == "__main__":
    ...
    p = argparse.ArgumentParser(description='Insert MIDI data into 837a08.7s (Gradius IV 68K Sound PRG ROM).')
    p.add_argument('input_file', type=str, help='Input file (e.g. 837a08.7s)')
    p.add_argument('input_directory', type=str, help='Input directory (e.g. midi)')
    p.add_argument('output_file', type=str, help='Output file (e.g. 837a08-modded.7s)')
    args = p.parse_args()
        ...
        for table_entry in range(table_entries):
            if (table_entry in dupe_table):
                ...
                if index_in_sort == 0:
                    ...
                    candidate_file = args.input_directory + '/midi' + hexu(table_entry) + '.mid'                    
                    if os.path.exists(candidate_file):                                         
                        print(hexu(table_entry) + ': exists, reading data into ' + hexu(pointer_table[table_entry]) + ' to ' + hexu(sorted_table[index_in_sort + 1]))
                        f3 = open(candidate_file, 'rb')
                        insert_data = f3.read()
                        f3.close()                        
                        for byteindex in range(len(insert_data)):
                            data2[pointer_table[table_entry] + byteindex] = insert_data[byteindex]
                        
        f4 = open(args.output_file, 'wb')
